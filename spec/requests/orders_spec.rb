...
describe "get order_path" do
  it "renders the :show template" do
    customer = FactoryBot.create(:customer)
    order_attributes = FactoryBot.create(:order, customer_id: customer.id) 
    get order_path(id: order_attributes.id)
...
describe "get new_order_path" do
    it "renders the :new template" do
      customer = FactoryBot.create(:customer)
      order_attributes = FactoryBot.create(:order, customer_id: customer.id) 
      get new_order_path(id: order_attributes.id)
...
describe "get edit_order_path" do
    it "renders the :edit template" do
      customer = FactoryBot.create(:customer)
      order_attributes = FactoryBot.create(:order, customer_id: customer.id)
      get edit_order_path(id: order_attributes.id)
...
### Having a hard time passing this test 
### I do not understand how line 88 works

# describe "put order_path with invalid data" do
#   it "does not update the order record or redirect" do
#     customer = FactoryBot.create(:customer)
#     order_attributes = FactoryBot.attributes_for(:order, customer_id: customer.id)
#     put order_path(order), params: {order: {product_name: 1}}
#     order.reload
#     expect(order.product_name).not_to eq(1)
#     expect(response).to render_template(:edit) 
#   end
# end
...
#Question about these routes test
#Do I need to input specific info inn lines 19 and 20?
#How do i specifically do a test int heterminal for the controller or model?s